  ...
  render () {
    ...
    return (
      <div className={cn(styles.patientList, this.props.className)}>
        <h2 className='mt-0'>Список пациентов</h2>
        <div className='my-3'>
          <button type='button' onClick={onAddClick}>Добавить</button>
        </div>
        <FindPatientByNameForm isPending={isPending} />
        <h3>
          {
            patientListSearchText ? `Поиск по запросу «${patientListSearchText}»` : 'Все пациенты'
          }
          {' '}
          ({total})
        </h3>
