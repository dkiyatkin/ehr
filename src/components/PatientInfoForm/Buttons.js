        ...
        <div className={cn(styles.right)}>
          {
            ...
            <button className={cn(styles.editBtn)} type='button' onClick={onEditClick} disabled={disabled}>Редактировать</button>
          }
          {
            isEditPatientInfoForm &&
            <React.Fragment>
              ...
              <button className={cn(styles.cancelBtn)} type='button' onClick={isNew ? onCloseClick : onCancelClick} disabled={disabled}>Отмена</button>
            </React.Fragment>
          }
          {
            (!isNew && !isEditPatientInfoForm) &&
            <button className={cn(styles.closeBtn)} type='button' onClick={onCloseClick}>Закрыть</button>
          }
        </div>
      </div>
    )
